...
function HotelCard({hotel}){
    return (
        <div to="" className="text-gray-600 bg-white rounded-md overflow-hidden  shadow-md">
            <Link to={`/room/${hotel.room._id}`}>
                <img src={hotel.room.images[0]} alt="roomImg" className="w-full" />
                <div className="p-5">
                    <div className="flex justify-between">
                        <div>
                            <h5 className="text-lg">{hotel.name}</h5>
                            <p className="text-sm w-48 truncate"><FontAwesomeIcon icon={faLocationDot} /> {hotel.address}</p>
                        </div>
                        <p><FontAwesomeIcon className="text-yellow-600" icon={faStar} /> 4.5</p>
                    </div>

                    <div className="flex justify-between items-center mt-3">
                        <p><span className="text-lg text-black">${hotel.room.price}</span>/night</p>
